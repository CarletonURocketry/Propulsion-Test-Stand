...
const uint16_t sendInterval = 100;
...
void setup() {
  ...
  LoadCell.begin();
  float calibrationValue = -43518.47; // calibration value (see example file "Calibration.ino")
...
void testStandControls() {
  ...
  for (int i = 1; i < numOfSolValve; i++) {
    digitalWrite(SolValvePins[i-1],bitRead(state_int,i));
  }
}
...
void readSensors() {
  ...
  if (LoadCell.update() and loadC) {
    L1Raw = max(0,LoadCell.getData() * 1000);
    Serial.println(L1Raw);
  ...
  data.P1 = adc.readADC_SingleEnded(0);
  data.P2 = adc.readADC_SingleEnded(1);
  //Serial.println(data.P3);

  if (thermC) {
    T1Raw = mcp.readThermocouple();
    data.T1 = static_cast<uint16_t>((round(T1Raw*1000)));
  } else {
    data.T1 = 300;
  }
