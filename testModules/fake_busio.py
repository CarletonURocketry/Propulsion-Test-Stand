...
from fake_adafruit_blinka import Enum, Lockable, agnostic
from fake_adafruit_blinka.agnostic import board_id, detector
...
class I2C(Lockable):
    ...
    def __init__(self, scl: Any, sda: Any, frequency: int = 100000) -> None:
        self.init(scl, sda, frequency)

    def init(self, scl: Any, sda: Any, frequency: int | None) -> None:
        """Initialization"""
        self.deinit()
        if detector.board.ftdi_ft232h:
            from adafruit_blinka.microcontroller.ftdi_mpsse.mpsse.i2c import I2C as _I2C # type: ignore

            self._i2c = _I2C(frequency=frequency)
            return
        if detector.board.binho_nova:
            from adafruit_blinka.microcontroller.nova.i2c import I2C as _I2C # type: ignore
        ...
        if detector.board.microchip_mcp2221:
            if (
                ...
                from adafruit_blinka.microcontroller.fake_mcp2221.i2c import I2C as _I2C # type: ignore
            else:
                from adafruit_blinka.microcontroller.mcp2221.i2c import I2C as _I2C # type: ignore
            self._i2c = _I2C(frequency=frequency)
            return
        if detector.board.greatfet_one:
            from adafruit_blinka.microcontroller.nxp_lpc4330.i2c import I2C as _I2C # type: ignore

            self._i2c = _I2C(frequency=frequency)
            return
        if detector.board.pico_u2if:
            from adafruit_blinka.microcontroller.rp2040_u2if.i2c import I2C_Pico as _I2C # type: ignore
        ...
        if detector.chip.id == ap_chip.RP2040:
            from adafruit_blinka.microcontroller.rp2040.i2c import I2C as _I2C # type: ignore

            self._i2c = _I2C(scl, sda, frequency=frequency)
            return
        if detector.board.any_siemens_iot2000:
            from adafruit_blinka.microcontroller.am65xx.i2c import I2C as _I2C # type: ignore

            self._i2c = _I2C(frequency=frequency)
            return

        if detector.board.any_embedded_linux:
            from adafruit_blinka.microcontroller.generic_linux.i2c import I2C as _I2C # type: ignore

            if frequency == 100000:
                frequency: Any = None  # Set to None if default to avoid triggering warning
    ...
    def readfrom_into(self, address: Any, buffer: Any, *, start: int = 0, end: None | int = None) -> Any:
    ...
    def writeto(self, address: Any, buffer: Any, *, start: int = 0, end: int | None = None) -> Any:
...
class SPI(Lockable):
    ...
    def __init__(self, clock: Any, MOSI: Any = None, MISO: Any = None):
        self.deinit()
        if detector.board.ftdi_ft232h:
